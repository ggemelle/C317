...
const PagePesquisaUser = ({ route }) => {
  ...
  const handlePesquisaSelecionada = (research_id, employeeId) => {

    console.log(`Pesquisa selecionada: ${research_id}`);
    navigation.navigate('pageRespondeUser', {research_id, employeeId});
  };

  const renderPesquisaItem = ({ item }) => (
    <TouchableOpacity
      style={styles.pesquisaItem}
      onPress={() => handlePesquisaSelecionada(item.research_id, employeeId)}
    >
